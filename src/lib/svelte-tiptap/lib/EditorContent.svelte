<script lang="ts">
	...
	const init = async () => {
		...
		Array.from(editor.options.element.childNodes).forEach((node) => element.appendChild(node));
	...
	onDestroy(() => {
		...
		Array.from(editor.options.element.childNodes).forEach((node) => newElement.appendChild(node));
